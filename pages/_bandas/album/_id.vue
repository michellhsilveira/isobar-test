    ...
    <header class="w-full bg-orange-600 h-16 bg-opacity-75 items-center px-4 fixed top-0 left-0 right-0 z-50">
      <div class="container items-center h-full justify-center">
        <div class="w-16 mr-auto">
          <nuxt-link :to="{ name: 'bandas-id', params: { bandas: band.id} }">
    ...
    <div class="container flex-col">
      ...
      <div class="flex flex-col" v-else>
        <div class="w-full flex h-64 bg-gray-500 bg-cover bg-center relative mb-24 overlay"
             v-bind:style="{ 'background-image': 'url(' + album.image + ')' }">
          <div class="mt-auto -mb-20 flex w-full flex-col text-center z-20">
            <h2 class="text-2xl text-white">{{ album.name }}</h2>
            <div class="flex items-center px-8">
        ...
        <div>
          <h3 class="text-2xl text-center">Lançado em {{ album.releasedDate | getAlbumYear }}</h3>
          <div class="w-full py-2 px-8" v-for="track in album.tracks">
            <div class="text-orange-600">{{ track.name }}</div>
            <div class="text-sm font-medium">{{ track.duration | formatDuration }}</div>
          </div>
        </div>
      </div>
    </div>
